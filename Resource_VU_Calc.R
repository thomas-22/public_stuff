...
dsp_resource_vu_func_single_val_VU_end <- function(n_start, VU_start, VU_end, cost_per_matrix = 3.3){
  lvls <- seq(VU_start-1, VU_end)
  n_eff <- numeric(length(lvls))
  n_abs <- numeric(length(lvls))
  eff_n_result <- cbind(lvls, n_eff)
...
n_start <- 100000 #absolute resource range start
n_end <-   5000000 #absolute resource range end
...
effective_values <- mapply(dsp_resource_vu_func_single_val_VU_end, grid$Var1, VU_start, grid$Var2)
data <- cbind(grid, effective_values)

# Plotting
fig <- plot_ly(data, x = ~Var1, y = ~Var2, z = ~log10(effective_values), type = 'scatter3d', mode = 'none')
fig <- fig %>% layout(scene = list(xaxis = list(title = 'Resource Amount Start', autorange = 'reversed'),
                                   yaxis = list(title = paste('VU lvl starting at lvl: ', VU_start)),
                                   zaxis = list(title = 'log10(Effective Amount)', 
                                                range = c(min(log10(effective_values)),
                                                          max(log10(effective_values))))))

# Adjust marker size
fig <- fig %>% add_markers(marker = list(size = 1))
